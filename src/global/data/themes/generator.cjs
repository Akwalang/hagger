...
const saveStyles = () => {
  ...
  for (const [name, { label, styles }] of Object.entries(themes)) {
    ...
    result.push({ name, label, file });
  }

  return result.sort((a, b) => a.name.localeCompare(b.name));
};

const saveExport = (files) => {
  let data = '';

  for (const { name, file } of files) {
    data += `import { styles as ${toCamelCase(name)} } from '${file}';\n`;
  }

  data += '\n';

  data += `export type Theme = {\n`;
  data += `  name: string;\n`;
  data += `  label: string;\n`;
  data += `  css: string;\n`;
  data += `};\n`;
  ...
  data += `export const themes: Theme[] = [\n`;

  for (const { name, label } of files) {
    data += `  { name: '${toSnakeCase(name)}', label: '${label}', css: ${toCamelCase(name)} },\n`;
  }
